...
describe('dsh web keyless CLI smoke', () => {
  it('serves a usable app from two immutable plugin batches', async () => {
    ...
    try {
      ...
      // Local fork: the printed URL carries no process token and the root
      // serves the app to a fenced requester directly.
      expect(readyUrl).toMatch(/^http:\/\/127\.0\.0\.1:\d+\/$/u)
      expect((await fetch(readyUrl, { redirect: 'manual' })).status).toBe(200)
