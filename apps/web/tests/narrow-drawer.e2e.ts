...
describe('web e2e: narrow drawer keeps the conversation centered', () => {
  ...
  it('the composer is visible and the details empty state is not', async () => {
    ...
    // The details column mounts (session scope) but must stay in the 0px
    ...
    expect(detailsBox?.width ?? 0).toBe(0)
    expect(await page.locator('[data-rightbar-collapsed]').count()).toBe(1)
  })
