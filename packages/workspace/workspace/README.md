...
- `ctx.workspaceRegistry.create(path, title?)` — canonicalizes `path` via `fs.realpath`, rejects a nonexistent or non-directory path, creates at most one record per canonical path, and prepends a new record to durable workspace order. Repeated calls for that path return the existing workspace without changing its title; different paths may share a display title. Created workspaces are local (`Workspace.place === { kind: 'local' }`).
- `ctx.workspaceRegistry.createAtPlace(place, path, title?)` — create or reuse a workspace for a place. A local place behaves exactly like `create`; an ssh place stores the destination with the remote absolute working path, keyed on host + remote path for reuse. The registry does not probe remote reachability (the local filesystem has no authority over a remote path) — a transport-aware consumer (e.g. the api gateway) connects and remote-stats the place before calling. `attachSession` for an ssh workspace matches the header cwd against the remote working path by exact equality.
- `Workspace.place` — the durable remote-ness statement: `{ kind: 'local' }` for directory workspaces, or `{ kind: 'ssh', host, user?, port? }` for remote ones. Records written before the field existed read as local. The working path is never part of the place: it lives once in `Workspace.path` (the remote absolute path for an ssh place).
...
- `ctx.workspaceRegistry.unarchiveSession(id)` — removes one session from the registry-global archive set: the session reappears on grouping surfaces at its retained accounting slot. The same existence rule as archiving applies (a typo'd id rejects whether or not the set holds it), and an id absent from the set resolves without writing.
...
- `Workspace.status()` — uncached place check, `'ok' | 'missing'`. A local place is a directory check; a remote place's reachability is the transport's concern, so this record-level probe reports `'missing'` until a transport-backed probe is composed in. A missing place never mutates the record.
