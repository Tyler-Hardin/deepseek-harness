...
describe('dsh-ssh-worlds', () => {
  ...
  it('appends an ssh/connect event when a session enters a remote world', async () => {
    ...
    const connects = session.snapshotEvents().filter(event => event.type === 'ssh/connect')
    ...
    expect(session.snapshotEvents().filter(event => event.type === 'ssh/connect')).toHaveLength(2)
    ...
    const disconnects = session.snapshotEvents().filter(event => event.type === 'ssh/disconnect')
