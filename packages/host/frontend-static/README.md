...
Serve the built Web shell to browsers from its configured distribution directory. The root and configured index path render the bootstrapped index; existing assets are served directly, while missing or non-file paths return 404, traversal returns 403, and unsupported methods return 405. Index access passes the Connection trust fence (local fork: no browser session exists), and static assets remain public. Only one instance can handle unmatched routes at a time; a second activation fails, and unloading the active instance makes unmatched requests return 404.
...
Root and configured-index responses call `ctx.connection.authorizeIndex` before reading HTML. Local fork: Connection owns no browser session, so that call always serves the index and no redirect, cookie, or 401 response exists. Non-index files remain public static assets, and Connection owns the request-trust fence every `/api` call passes.
