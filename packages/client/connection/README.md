...
- [Request trust (local fork: no browser authentication)](#request-trust-local-fork-no-browser-authentication)
...
The browser uses HTTP POST for Remote unary calls. API Gateway owns the `/api/remote.mux` WebSocket and its logical streams; shell-owned compositions provide equivalent Remote streams through `connection.rpc.open` without opening a WebSocket. The Host half always provides the carrier-neutral RPC and exact `GET`/`HEAD`/`POST` route registries. When a Web carrier is present it also owns the sole `/api` route, Fetch bridge, and Host/Origin checks; a shell-owned carrier dispatches the shared Fetch handler directly. Each exact route declares buffered or streaming request-body handling before the bridge reads any bytes. Typert Gateway claims generated Remote endpoints, feature packages register non-JSON responses such as Session-log downloads and raw file uploads, and unclaimed requests return 404. Loopback hostname classification remains package-internal to the browser-facing Client state. Browser raw-body transfer is provided by [`dsh-client-file-upload`](../file-upload/README.md).

-----

<a id="request-trust-local-fork-no-browser-authentication"></a>
## Request trust (local fork: no browser authentication)

Every Host RPC method and WebSocket stream passes `src/api-request-trust.ts` and nothing else. Its `Host` must be loopback or match a `trustedHosts` entry: exact on `host:port`, any port on port-less entries, both sides WHATWG-normalized. An attached `Origin` must equal that Host and `sec-fetch-site: cross-site` is refused. Malformed configured authorities fail plugin load. These checks defend DNS rebinding and cross-site browser requests; they never establish identity. A failed Host/Origin check returns 403, and static assets stay public.

This fork removed the browser session entirely: no process launch token, no cookie, no `cookieMaxAgeDays`, and no `credentials` injection. `frontend-static` still delegates root and index requests to `ctx.connection.authorizeIndex`, which now always serves them, and `dsh-web-app` prints and opens the plain deployment URL. Authentication is therefore the deployment's own: reach this Host only through a reverse proxy that terminates TLS and verifies the client certificate, and declare that proxy's authority in `trustedHosts`. Anyone the fence admits reaches every method, the settings plane included. `dsh web --host 0.0.0.0` remains unsupported. Decision record for the fence itself: [browser request trust](../../../.agents/notes/implemented/architecture/2026-07-28-api-browser-trust-boundary.md).
