...
import {
  SandboxRootsSettingsController, extraRootsOf, type SettingsWrite,
...
/** The sandbox controller over a real mirror and one fake wire namespace. */
function sandboxController(api: { describe: () => Promise<unknown>; mutate: (...args: unknown[]) => Promise<unknown> }) {
  const ctx = { remote: { settings: api } } as never
  const mirror = new SettingsDescribeMirror(ctx)
  const write: SettingsWrite = async (ns, ops, expectedRevision) => {
    const response = await api.mutate(ns, ops, expectedRevision) as {
      readonly ok: boolean
  ...
  return { mirror, controller: new SandboxRootsSettingsController(mirror, write) }
}
...
describe('sandbox roots settings store', () => {
  ...
  it('contains read failures and no-ops without a writable view', async () => {
    ...
    const stringReject = sandboxController({
      describe: () => Promise.resolve(ok({ writable: true, hasDocument: false, namespaces: [view(['/cache'])] })),
      mutate: () => Promise.reject(new Error('wire exploded')),
    }).controller
