...
/** One namespace write as this row issues it: path operations plus the revision fence. */
export type SettingsWrite = (
  ...
  | { readonly kind: 'conflict' | 'refused'; readonly message: string }
>
...
export class SandboxRootsSettingsController {
   ...
   * @param write - namespace write for the `extraWritableRoots` replacement.
   */
  constructor(
  ...
  async save(roots: string[]): Promise<void> {
    ...
    try {
      const answer = await this.write(
        SANDBOX_SETTINGS_NS,
        [{ op: 'set', path: ['extraWritableRoots'], value: roots }],
        view.revision,
      )
      if (answer.kind !== 'written') throw new Error(answer.message)
      this.saving = false
      if (this.disposed) return
      // The mirror publish reaches this row's own subscription, so the fold
      // is also what republishes the accepted value here.
      this.describeFace.acceptView(answer.view)
    } catch (error) {
      this.saving = false
      if (this.disposed) return
      this.fail(error)
