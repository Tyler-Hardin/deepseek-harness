 ...
 * to the composer draft through the standard `useInput` value and
...
export interface VoiceInputInjected {
  ...
  /** Mic control copy, bound to this plugin's dictionary namespace. */
...
export function VoiceInput({ useInput, inputActions, transcribe, t }: VoiceInputProps) {
  const input = useInput(state => state)
  const [phase, setPhase] = useState<Phase>('idle')
  const [error, setError] = useState<string | null>(null)
  const recorderRef = useRef<VoiceRecorder | null>(null)

  const label = t('mic.label')
  ...
  }, [phase, input.draft, inputActions, transcribe])

  const title = phase === 'error' && error !== null ? t('mic.title.error', { label, error }) : label
