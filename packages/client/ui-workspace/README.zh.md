...
已归档会话汇聚在分组视图末尾、Ungrouped 之后的**「已归档」**分区（平铺列表与内容搜索继续排除它们）：每行显示标题、相对时间与状态，其行菜单只提供一项操作——**恢复会话**——调用 `ctx.workspaces.unarchiveSession`，归档集合回声落地后该行按保留的记账席位回到所属 workspace 分组（或 Ungrouped）。点击已归档行绝不会打开它：投影层会把被归档的当前 selection 清空回 New Session 视图，因此该行除菜单外保持惰性。空白占位符与 subagent 来源的行不会进入该分区（空白行没有可恢复的内容，subagent 子会话使用其父级的头部目录）。该分区首次出现时默认展开，其展开状态与其它分组一样持久化。
...
- **没有 Session 删除控件**：删除 Workspace 注册记录不会删除 Session。已归档会话可在侧边栏的「已归档」分区（分组视图）中查看与恢复。
