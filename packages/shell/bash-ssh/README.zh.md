---
description: "`ctx.shell` seam 的 SSH 执行器：面向在远程执行世界上运行命令的部署方与维护者。"
...
使用 `dsh-bash-ssh` 可经 [`@deepseek-ai/dsh-ssh`](../../ssh/ssh/README.zh.md) world 的 exec 通道在一个远程执行世界上运行命令。`SshBashExecutor` 用 shell seam 的 exec/collect 生命周期运行前台命令——配置钳制的超时、取消、有界输出、stdin 与环境——并在远端主机上分离地运行后台进程。它负责远端一侧与 bash 相关的行为：命令默认值与上限、超时与取消分类、适合模型的终端环境，以及面向模型的 stdout/stderr 合并。传输机制属于 SSH seam，且一个实例服务于一个远端 world。宿主上请选择 `bash-local`，混合组合请选择 `shell-router`。
...
`dsh-tool-bash` 渲染该执行器的有界 stdout/stderr 尾部、后台进程增量与基础设施故障；SSH 传输与远端 `setsid` wrapper 保持内部。启动失败或连接中断会以一条经 `readOutput()` 单次投递的说明到达模型。

#### Token 影响

只有被渲染的输出消耗 token：前台捕获与后台尾部按流受 `maxOutputBytes` 约束，被截断的后台尾部标记为 `lossy`，而不会被展开。
...
**运行时不变式：** 不发布伴生入口。每次操作直接返回世界 exec 通道已提交的结果，没有独立的事件序列或缓存可供交叉校验；世界的连接生命周期属于 SSH seam。
