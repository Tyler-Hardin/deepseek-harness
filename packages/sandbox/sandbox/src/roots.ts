 ...
 * workspace root, the platform temp areas, and the policy's configured extra
 * writable roots", and this module is that meaning's one home. The Seatbelt
 * profile (`@deepseek-ai/dsh-sandbox-local`) and the in-process filesystem
 * fence (`@deepseek-ai/dsh-fs-sandbox`) both derive their allow-list here, so
 * "the write tool cannot write /tmp but bash can" asymmetries cannot arise
 * between them. The bwrap and Landlock dialects keep their own grant
 * spellings (an ephemeral `/tmp` mount, launcher-owned flags) — the honest
 * per-runner differences recorded in the sandbox RFC — with parity pinned by
 * test.
 *
 ...
 * the policy's configured extra writable roots.
...
export function writableRoots(policy: SandboxExecutionPolicy): string[] {
  ...
  return [...new Set([policy.workspaceRoot, '/tmp', tmpdir(), ...(policy.extraWritableRoots ?? [])].map(canonicalPath))]
}
