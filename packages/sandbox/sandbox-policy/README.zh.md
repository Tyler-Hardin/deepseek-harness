...
使用本包可以让每次受限的 bash、文件系统和终端调用遵循同一份文件操作策略。部署方选择默认模式、回退工作区根目录以及可选的宿主机本地额外可写根目录，每个会话则可以独立切换模式。会话选择可跨重启保留，所有强制执行能力在一次调用中使用相同的模式和工作区。每次模型请求前，模型都会收到有效策略和工作区说明，但不会收到已挂载能力的清单。
...
| `extraWritableRoots` | `[]` | 除会话工作区与平台临时区域外，`workspace-write` 还可写入的宿主机本地绝对目录（例如 `~/.cache`；开头的 `~` 展开为用户主目录，非绝对拼写会在加载时被拒绝）。远程执行世界永远不会收到它们 |

生成的[配置目录](../../../docs/config-catalog.zh.md#deepseek-aidsh-sandbox-policy)是每个受支持字段及其 JSDoc 的穷尽式真源。

### 配置额外可写根目录

`extraWritableRoots` 在会话工作区与平台临时区域之外授予长期写入权限，用于工作流确实需要的宿主机本地目录（构建缓存、共享检出）。当会话工作区应当就是全部可写范围时保持为空。用户层 `sandbox` 设置命名空间会覆盖这份部署默认值：存储的段落会整体替换该列表，且存储的变更无需重启即可通过 `resolve()` 到达下一次受限调用。
...
`resolve({ session, mode })` 返回一份完整的逐调用策略：已批准的显式模式优先于会话最后一条 `sandbox/mode` 事件，后者又优先于部署默认值。会话的不可变 `cwd` 先按文件系统语义规范化，再成为工作区根目录，因此 `symlink/..` 与进程工作目录解析一致；否则使用配置的回退值。有效的额外可写根目录会规范化、去重，并仅在列表非空时写入策略。
...
- **每个会话只有一个主要工作区根目录**——策略解析 `SessionHeader.cwd`，受限子集只携带该根目录；`extraWritableRoots` 是并列的额外授权，而不是第二个根目录。
- **仅限文件操作模式**——`SandboxMode` 管控文件操作；网络和进程策略不在其词汇中，因此这里没有限制它们的旋钮。
- **有意概述临时区域**——强制执行后端会授予不同的平台临时区域，这些区域在策略解析后才会选定，因此无法在当前上下文中如实枚举。
- **按约定，额外根目录仅限宿主机本地**——远程执行世界永远不会收到它们，因此该列表无法授权另一台主机上同名路径；按主机的远程策略暂缓至远程限制相关工作。
