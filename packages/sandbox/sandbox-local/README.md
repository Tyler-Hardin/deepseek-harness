...
The bwrap profile combines a read-only host root, fresh `/dev`, and private-PID `/proc`. Commands manage descendants but cannot see host processes; hiding host `/proc/<pid>` entries prevents magic links such as `root` and `fd` from bypassing its mounts. `workspace-write` adds ephemeral `/tmp`, a writable workspace bind, and one writable bind per configured extra writable root. The [private-PID Agent Note](../../../.agents/notes/implemented/bug-fix/2026-08-06-bwrap-private-pid-namespace.md) records the boundary.

The Seatbelt profile is allow-default with `(deny file-write*)` plus write allow-lists, so exactly the mode's promised file effects are governed: `read-only` grants the `/dev/null` literal alone; `workspace-write` adds the workspace root, `/tmp`, the per-user darwin temp dir (`os.tmpdir()` — the platform's real temp area for mkstemp-family tools), and every configured extra writable root, all roots canonicalized because Seatbelt matches resolved paths (`/tmp` IS `/private/tmp`). Apple marks the `sandbox-exec` CLI deprecated but ships it on every macOS; the functional probe is what fails closed if that ever changes.
...
- **The windows-acl rung grants workspace and private temp only** — configured extra writable roots get ACEs on the POSIX runners (bwrap/Landlock binds, Seatbelt subpaths) but not on the ACL runner, where a write to an extra root is denied. The fs fence derives from the same roots, so on Windows the write tool may permit an extra root the shell runner denies; per-root ACE grants are deferred.
