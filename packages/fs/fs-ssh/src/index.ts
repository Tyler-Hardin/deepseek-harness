...
export class SshFileSystem extends FileSystem {
  ...
  override async readByteRange(
    ...
    try {
      const sftp = await this.sftp()
      // `start`/`end` are 0-based inclusive on the wire, so a requested
      // `length` of zero must not become a one-byte read.
      const stream = sftp.createReadStream(String(target.targetKey), {
        start: range.offset,
        end: range.offset + Math.max(range.length, 1) - 1,
      })
      const chunks: Buffer[] = []
      let bytes = 0
      for await (const chunk of stream as AsyncIterable<Buffer>) {
        ...
        chunks.push(chunk)
        bytes += chunk.length
        if (bytes >= range.length) break
      }
      return Buffer.concat(chunks, bytes).subarray(0, range.length)
    } catch (error: unknown) {
