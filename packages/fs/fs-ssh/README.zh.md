---
description: "`ctx.fs` 的 SSH/SFTP 后端：面向为一个远程执行世界提供文件的部署方与维护者。"
...
使用 `dsh-fs-ssh` 可经 SFTP 通过 [`@deepseek-ai/dsh-fs`](../fs/README.zh.md) seam 服务一个远程执行世界。路径、内容与原子暂存文件都留在远程主机上；读取暴露常规 UTF-8 文本或类型化错误，目录列表稳定且不含内容，变更原子化并带可选版本守卫。提供方接收来自 [`@deepseek-ai/dsh-ssh`](../../ssh/ssh/README.zh.md) 的 `SshWorld`，把接缝的暂定 `SftpHandle` 固定为 ssh2 包装器，并且每个实例服务一个远程世界；工作区/会话绑定阶段按远程工作区组合实例。宿主文件请选择 `fs-local`，同一组合跨多个世界时选择 `fs-router`。
...
`dsh-tool-fs` 渲染远程 UTF-8 内容、目录结果、变更确认与提供方错误，SSH 传输保持内部。失败以接缝的类型化错误码到达模型——`FS_NOT_FOUND`、`FS_PERMISSION_DENIED`、`FS_IO_ERROR` 与 `FS_ABORTED`——SSH 错误作为 cause 保留。

#### Token 影响

远程结果本身是唯一的模型可见开销：一次读取、列表或变更确认只贡献其返回的内容，并受消费工具的保留上限约束。SFTP 属性细节留在提供方内部。
...
**运行时不变式：** 不发布伴生入口。每次操作直接返回 SFTP 传输已提交的结果，没有独立的事件序列或缓存可供交叉校验；世界的连接生命周期属于 SSH seam。
