    ...
    let
      ...
      dsh =
        ...
        let
          ...
          # source with the same flags as native/system/scripts/build.ts for the
          # `static-musl` launcher (-static against musl). Linux-only; the thunk
          # is only forced by the guarded installPhase below.
          landlockRun = pkgs.pkgsMusl.stdenv.mkDerivation {
            pname = "landlock-run";
            version = (builtins.fromJSON (builtins.readFile ./native/system/package.json)).version;
            src = self + "/native/system/packages/entry/src";
            dontConfigure = true;
            buildPhase = ''
              mkdir -p build
              cc -std=c11 -Os -Wall -Wextra -Werror -static -s -o build/landlock-run main.c
            '';
            installPhase = ''
              mkdir -p $out/bin
        ...
        pkgs.stdenv.mkDerivation {
          ...
          installPhase = ''
            ...
            ${pkgs.lib.optionalString pkgs.stdenv.hostPlatform.isLinux ''
              ...
              # native/system/packages/linux-x64/bin/landlock-run, reached
              # through the platform package `@deepseek-ai/node-addon-system-linux-x64`
              # that the entry package's node_modules links back to.
              mkdir -p $out/lib/dsh/native/system/packages/linux-x64/bin
              cp ${landlockRun}/bin/landlock-run \
                $out/lib/dsh/native/system/packages/linux-x64/bin/landlock-run
            ''}
            cat > $out/bin/dsh <<EOF
            #!${pkgs.runtimeShell}
            ${pkgs.lib.optionalString pkgs.stdenv.hostPlatform.isLinux ''
    ...
    in
    {
      ...
      devShells = forAllSystems (
        ...
        let
          ...
          # Python must be on PATH. native/system's launcher builds with
