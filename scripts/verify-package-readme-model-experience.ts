...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/ssh/ssh': { kind: 'none', reason: 'Transport seam only: it opens connections and channels, registering no prompt, tool schema, or transcript content.' },
  'packages/ssh/ssh-client': { kind: 'none', reason: 'Provider for the SSH seam: it implements the transport contract and registers nothing model-facing of its own.' },
  'packages/ssh/ssh-worlds': { kind: 'none', reason: 'Provider that composes the SSH transport into execution worlds; the fs/shell routers own any model-visible rendering.' },
