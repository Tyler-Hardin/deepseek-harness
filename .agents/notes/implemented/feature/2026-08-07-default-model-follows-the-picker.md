...
`selectionFor(agent)` resolves its tiers on every read: a process-local session selection, otherwise the session's latest logged `request/header`, otherwise the live Agent default. A session with a logged request remains bound to that durable selection. A blank session observes the current default even when it was created before the preference was saved; this matches the New Session surface, which may reuse a blank session. [Per-workspace default model](2026-08-25-per-workspace-default-model.md) extends the tier order with an explicit per-workspace override between the logged selection and this shared default, and routes `session.selectModel`'s save to the workspace override when the session's workspace carries one.
