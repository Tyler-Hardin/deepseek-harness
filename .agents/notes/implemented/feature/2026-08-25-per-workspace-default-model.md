...
The gateway gains the `workspace.defaultModel` / `workspace.setDefaultModel` RPC pair, the settings document gains a `workspace-default-model:` section keyed by workspace id, and the composer's model seat keeps saving the shared default only for sessions whose workspace carries no override. `agent-default-model`'s service API and READMEs, the apiproxy READMEs, and the browser workspace-menu copy (zh/en) document the new surface. This note extends — and partially supersedes — [the default model follows the picker](../../archived/feature/2026-08-07-default-model-follows-the-picker.md), which owns the shared-default persistence this tier builds on.
