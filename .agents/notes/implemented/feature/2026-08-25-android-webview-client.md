...
Page-triggered native notifications (`window.DshApp.notify()`) fire while the app process is alive. Background task-completion notifications are delivered by the later foreground-service monitor — [android task-completion notifications](2026-09-07-android-task-completion-notifications.md) — which keeps its own downlink sockets and never depends on the WebView page. The app trades goop's invisible failure modes for a few more visible surfaces: the web UI's App settings page with diagnostics, the offline fallback screen, and a crash dialog. Settings changes now ship in the `dsh web` server (the App settings page) rather than only in the APK. The app builds with the same Android toolchain as the goop app (AGP 8.2.0, Kotlin 1.9.22, compileSdk 34, Gradle from nixpkgs), so the proven build path carries over.
