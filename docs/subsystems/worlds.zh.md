...
[dsh-fs-router](../../packages/fs/fs-router) 与 [dsh-shell-router](../../packages/shell/shell-router) 是产品消费者：它们读取调用方的不透明世界身份（`opts.world` / `request.world`，在会话创建时冻结于 [`SessionHeader`](persistence.zh.md#sessionheader--metadata-beside-the-log)）并把每次调用分发到已解析世界的后端。[dsh-api-workspace-controller](../../packages/api/workspace-controller) 在创建会话时从工作区位置解析会话的世界。
