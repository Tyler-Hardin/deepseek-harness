...
[dsh-fs-router](../../packages/fs/fs-router) and [dsh-shell-router](../../packages/shell/shell-router) are the product consumers: they read the caller's opaque world identity (`opts.world` / `request.world`, frozen in the [`SessionHeader`](persistence.md#sessionheader--metadata-beside-the-log) at session creation) and dispatch each call to the resolved world's backend. [dsh-api-workspace-controller](../../packages/api/workspace-controller) resolves a session's world from its workspace place when the session is created.
