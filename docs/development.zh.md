...
- Nix 用户可跳过手动安装工具，通过 `nix develop` 进入固定工具链（Node.js 24、Corepack 固定的 pnpm、原生构建依赖、uv）；`nix build` 构建本仓库的 dsh 包，[`services.dsh-web`](../nix/dsh-web.nix) NixOS 模块将其作为 systemd 服务运行。
